function prestigeButtonText(layer) {
	...
	if (tmp[layer].type == "normal")
		return `${player[layer].points.lt(0) ? (tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : "重置以获得") : ""}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource} ${tmp[layer].resetGain.lt(100) && player[layer].points.lt(0) ? `<br><br>下一个在: ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : format(tmp[layer].nextAt))} ${tmp[layer].baseResource}` : ""}`
